...
SELECT * FROM actor a WHERE a.actor_id IN (SELECT f.actor_id FROM film_actor f WHERE f.film_id IN 
(SELECT fm.film_id FROM film fm WHERE fm.title = 'BETRAYED REAR') AND a.actor_id IN (SELECT f.actor_id FROM film_actor f WHERE f.film_id IN (
SELECT fm.film_id FROM film fm WHERE fm.title = 'CATCH AMISTAD')))

SELECT * FROM actor a WHERE a.actor_id NOT IN (SELECT f.actor_id FROM film_actor f WHERE f.film_id IN 
(SELECT fm.film_id FROM film fm WHERE fm.title = 'BETRAYED REAR')) AND a.actor_id NOT IN (SELECT f.actor_id 
FROM film_actor f WHERE f.film_id IN (SELECT fm.film_id FROM film fm WHERE fm.title = 'CATCH AMISTAD'));
